...
上面的快速幂思想虽然大幅减少了累乘次数，但是每一位还是要做指数级别的累乘，uniswapV3 用预计算常量做到极致的优化。我们先用快速幂的思想拆解这个公式： $\text{sqrtPriceX96} = 1.0001 ^{\frac{i}{2}} \cdot 2^{96}$ ，可得：
...
- $m-a$ ：m 到左锚点 a 的距离
- $b-a$ ：两个锚点之间的总距离（uniswapV3 中相邻两个锚点距离为 0.0625）
